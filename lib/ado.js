...
async function fetchBuildStatus({ organization, project, buildId, token }) {
  ...
  let normalized = normalizeStatus(data.status, data.result, 'Build');
  // A YAML stage waiting on an environment approval keeps the build itself "inProgress",
  // so the pending checkpoint in the timeline is the only way to see it.
  if (normalized.status === 'running') {
    const waiting = await fetchBuildPendingApproval({ organization, project, buildId, token });
    if (waiting) normalized = waiting;
  }
  const webUrl =
...
// Returns an awaitingApproval status when a stage is blocked on an approval check, else null.
async function fetchBuildPendingApproval({ organization, project, buildId, token }) {
  let records;
  try {
    const timeline = await fetchJson(
      `https://dev.azure.com/${encodeURIComponent(organization)}/${encodeURIComponent(
        project
      )}/_apis/build/builds/${encodeURIComponent(buildId)}/timeline?api-version=${API_VERSION}`,
      token,
      'Build timeline was not found.'
    );
    records = timeline.records || [];
  } catch {
    // Timeline is a nice-to-have; never fail a refresh over it.
    return null;
  }
  const pending = records.filter(
    (record) => record.type === 'Checkpoint.Approval' && record.state === 'inProgress'
  );
  if (!pending.length) return null;

  const byId = new Map(records.map((record) => [record.id, record]));
  const stageNames = [...new Set(pending.map((record) => {
    let current = record;
    while (current && current.type !== 'Stage') current = byId.get(current.parentId);
    return current?.name;
  }).filter(Boolean))];

  return {
    status: 'awaitingApproval',
    message: stageNames.length
      ? `Awaiting approval to start ${stageNames.join(', ')}.`
      : 'Awaiting approval before the next stage can start.'
  };
}
